		...
		if(myObjectClick.disabled != true){
		    ...
		    if(myObjectClick.value != ''){
			    //On utilise la chaine de separateur
					...
					calendrier( tableau[0],	tableau[2] - 1, tableau[1]);
		    }
		    else{
...
function alimenterChamps(daySelect){
		if(daySelect != ''){
			lastInput.value= current_year + separateur + formatInfZero(daySelect) + separateur + formatInfZero((current_month+1));
		}
		else{
			lastInput.value = '';
		}
...
function CheckDate(d) {
      // Format de la date : AAAA-JJ-MM .
      var j=(d.substring(5,7));
      var m=(d.substring(8));
